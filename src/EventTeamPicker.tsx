  ...
  const gridChildren = entries?.[meet]?.[evt!]?.entrants.map((entrant, i) => {
    ...
    return (
      <AthleteCard
        ...
        avatar={`img/${pixelMode ? 'avatars' : 'hqAvatars'}/${id}_128x128.png`}
        meet={meet}
        event={evt!}
                ...
                <Tooltip.Group openDelay={0} closeDelay={100}>
                  <Avatar.Group spacing="xs">
                    {myTeamPicks.map(({ id, lastName }, i) => (
                      ...
                      >
                        <Avatar size={i === 0 ? 'lg' : i < PICKS_PER_EVT ? 'md' : 'sm'} src={`img/${pixelMode ? 'avatars' : 'hqAvatars'}/${id}_128x128.png`} radius="xl" />
                      </Tooltip>
                    ))}
                  </Avatar.Group>
                </Tooltip.Group>
